...
import {
  loadMemory, saveMemory, memoryToPrompt, distill,
} from './memory.js';
...
async function rememberTurn({
  visitorId, prior, userMsg, assistantMsg, leadFacts,
}) {
  if (!visitorId) return;
  try {
    const updated = await distill({
      client, model: MODEL, prior, userMsg, assistantMsg, leadFacts,
    });
...
// Execute one model-proposed tool call (arguments validated server-side) and
// shape the result as the `tool` message the model expects. Also surfaces the
// lead details when crear_lead succeeded, so they can be folded into memory.
async function runToolCall(call, locale) {
  let args = {};
  try {
  ...
  const result = await executeTool(name, args, { locale });
  return {
    message: { role: 'tool', tool_call_id: call.id, content: JSON.stringify(result) },
    leadFacts: name === 'crear_lead' && result?.ok ? args : null,
  };
}

// One model round-trip, recursing while the model keeps requesting tools.
// On the final allowed round, tools are disabled so the model must answer in
// text — the recursion is therefore bounded by MAX_TOOL_ROUNDS (cost/latency
// guard: a misbehaving model can never loop forever).
async function runToolRounds({
  convo, round, locale, leadFacts,
}) {
  ...
  if (calls.length === 0) {
    return { reply: msg?.content?.trim() || '…', leadFacts };
  }

  // The model proposed one or more tool calls. Record its turn, then execute
  // them (independent of each other, so in parallel) and append the results.
  convo.push(msg);
  const outcomes = await Promise.all(calls.map((call) => runToolCall(call, locale)));
  outcomes.forEach((outcome) => convo.push(outcome.message));
  const capturedLead = outcomes.map((outcome) => outcome.leadFacts).find(Boolean) ?? leadFacts;

  return runToolRounds({
    convo, round: round + 1, locale, leadFacts: capturedLead,
  });
}
...
export async function generateReply({ messages, locale, visitorId }) {
  ...
  // Tool-calling rounds: the model may ask to run tools (trip 1); we execute
  // them, feed the results back, and let it answer (trip 2). `leadFacts` are
  // the structured facts captured for free when crear_lead succeeds — folded
  // into the distilled memory (the "hybrid" half that costs nothing extra).
  const { reply, leadFacts } = await runToolRounds({
    convo, round: 0, locale, leadFacts: null,
  });

  await rememberTurn({
    visitorId, prior: memory, userMsg: lastUserMsg, assistantMsg: reply, leadFacts,
  });
  return reply;
}
